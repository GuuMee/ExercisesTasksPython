...
parameter. Your function should return a list of the words in the string with the punctu�ation marks at the edges of the words removed. The punctuation marks that you must
...
the middle of a words, such as the apostrophes used to form a contraction. For exam�ple, if your function is provided with the string "Examples of contractions
...
def only_words(s):
    dic = s.maketrans('', '', ''.join(PUNCTUATIONS))
    new_s = s.translate(dic)
    words = new_s.split(" ")
    return words


def main():
    line = input("Enter the string to identify the words: ")
    words_list = only_words(line)
    count = len(words_list)
    print("Your string has", count, "words")
    print("The list of words:", words_list)
